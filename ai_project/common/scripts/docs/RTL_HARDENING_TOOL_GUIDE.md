...
| 寄存器数 | 加固后寄存器总数 | AST分析 |
| 信号数 | 已加固信号总数 | 策略路由结果 |
| 面积开销 | 相对原始设计的面积增加比 | 策略加权计算 |
| 可靠性 | 加固后可靠度（0~1） | 策略保护因子 |
| 延迟开销 | 增加的延迟周期 | 策略特性评估 |
...
| Tab | 内容 | 用途 |
|:---|:---|:---|
| **原始代码** | 未修改的 RTL 源码 | 对比基准 |
| **加固后代码** | TMR/Parity/ECC 等加固后的 RTL | 查看实际变换结果 |
| **LLM生成代码**（可选） | LLM 生成的备选加固代码 | 对比不同策略 |
| **代码对比高亮** | 加固变换的 reg/wire 声明和赋值 | 快速定位变更点 |

#### 4.4 高级分析结果

| 面板 | 内容 | 自动化程度 |
|:---|:---|:---|
| **策略分配详情** | 每个信号的 `[类型] → 策略` 映射 | 自动显示 |
| **脆弱性评分 Top 5** | 最高风险寄存器的评分（红色 >0.7） | 自动显示 |
| **高扇出信号** | 扇出计数排名（橙色标注） | 自动显示 |
| **AIG电路分析** | AND门数 / PI/PO / 高扇出节点 | 默认开启，自动执行 |
| **故障注入验证** | 注入次数 / 加固前AVF / 加固后AVF / 改善幅度 | 默认开启 |

#### 4.5 验证评估量化指标示例

```
[VERIFY] 多层验证: 4层通过, 0个警告, 0个失败
[COMPILE] iverilog编译检查: ✅ 编译成功 (0 errors, 0 warnings)
[AIG] AND门=245, PI=32, PO=16, 高扇出节点=5
[FAULT] 完成: 平均AVF=0.042, 加固改善=88.0%
```
...
| 测试套件 | 用例数 | 结果 | 验证内容 |
|---------|-------|------|---------|
| 全流程测试 (test_workflow.py) | 3套件 | ✅ PASS | 单文件/文件夹/数据集三种流程，含内联TMR变换+时钟自动检测 |
| 增量加固测试 (test_incremental_hardening.py) | 33 | ✅ PASS | 信号解析/差异检测/增量更新/补丁生成/验证 |
| GUI自动化测试 (test_gui.py) | 7 | ✅ PASS | 导入/类加载/管线集成/历史/工作流/配置/一键升级 |
| 插件系统测试 (plugin_manager.py) | 1插件 | ✅ PASS | 动态加载/ABFT策略应用/启用禁用 |
| yosys可用性 | 1 | ✅ D:\oss-cad-suite\bin\yosys.exe |
| 功能完整性验证 | 8项 | ✅ PASS | 逻辑锥/日志/配置/时钟检测/内联TMR/OR-tree/投票器/CDC |

**关键功能验证日志 (v5.1)**:
```
[DETECT] 时钟: clk, 复位: rst_n                          # 时钟自动检测
[TRANSFORM] state → tmr (内联, clk=clk, rst=rst_n)       # 内联TMR
[OUTPUT] 已添加 keep 属性到 reg 声明                       # 综合保护
[ERROR_OR] 生成OR-tree: 9 个信号汇聚到 tmr_global_error   # OR-tree
[LOGIC_CONE] 逻辑锥验证: 5锥, PASS                         # 逻辑锥验证
[CONFIG] 配置已保存: config/pipeline_config.json            # 配置持久化
[PLUGIN] ✅ 加载: abft_extension v1.0                      # 插件加载
[GNN_TRAIN] 训练完成: acc=0.92, f1=0.89                   # GNN训练
```
...
```
v5.4 已完成优化项:
├── Phase 0: 注释约束 / 综合保护 / 故障知识积累              ✅
├── Phase 1: AST变换修复(内联TMR+时钟检测)                   ✅
├── Phase 2: FSM扩展(fsm_hamming/fsm_safe) / OR-tree / DICE变体 ✅
├── Phase 3: 投票器多样性 / CDC检测 / PPA多目标评估           ✅
├── GUI增强: 一键升级 / 配置持久化 / 对比表 / 错误信号开关    ✅
├── P3新增: GNN训练环境 / 插件系统 / 日志分级 / 逻辑锥验证   ✅
├── v5.3新增: 投票器三重复制 / 禁区规则 / 多层验证 / 双知识库RAG ✅
├── v5.4新增: BCH码多比特纠错                                  ✅
└── 工具完备性: 13策略 + 4投票器 + 8功能模块 + 1示例插件    ✅
```

### 6.3.2 调研报告中仍保留的未实现创新点

以下是从《RTL级加固方法综述与技术选型报告》中梳理出的、**在当前工具中仍未实现**的创新点（v5.4已实现34/37项）：

| 来源 | 未实现创新点 | 说明 | 参考价值 | 工作量 |
|------|------------|------|---------|-------|
| **TMRG** | **PLAG布局约束生成** | 生成物理布局约束防止TMR单元间距过近 | **低** — 物理设计层面，超出RTL工具范畴 | — |
| **DICE文献** | **HLTNURL三节点自恢复** | 最高级别DICE变体，三节点翻转恢复 | **低** — TNUDICE已实现类似保护 | — |

#### 已实现的补充说明

- ✅ **BCH码多比特纠错** (v5.4已实现): 基于GF(2^m)域的完整BCH(15,11,1)/(15,7,2)/(31,26,1)/(31,21,2)/(31,16,3)编解码实现，支持1-3比特错误纠正。

#### 不采纳原因说明

| 创新点 | 不采纳原因 |
|--------|-----------|
| **PLAG布局约束** | 布局约束属于物理设计(FPGA P&R)阶段的工作，RTL级工具无法产生有效的物理约束。TMRG使用PLAG是因为它集成了Xilinx设计套件。在本工具中，用户可以在综合后使用Xilinx Vivado等工具的TMR功能生成布局约束。 |
| **HLTNURL三节点** | 当前工具已实现TNUDICE(三节点DICE)，HLTNURL是TNUDICE的变体，保护机制类似。在缺乏硅验证数据的情况下，增加此策略不会带来实质性的保护提升。 |

### 6.3.3 当前状态总结

```
v5.4 已达到 92% 创新点覆盖率 (34/37项)。

剩余2项未实现:
├── PLAG布局约束生成    — 物理设计阶段，超出RTL工具范畴
└── HLTNURL三节点自恢复 — TNUDICE已实现类似保护
...
> 当前工具功能完备度 **99%**（功能覆盖）/ **92%**（创新点覆盖，v5.4）。
> 调研报告37个核心创新点中，已实现34个，剩余2个待实施（1个部分实现）。

---

# 第七章: 创新点来源与借鉴分析

> 本章基于对TMRG/CERN、TaMaRa/YosysHQ、FT-Pilot/中科院计算所等开源工具和论文的调研，
> 分析各工具的独特创新点，评估其对当前工具优化的参考价值。

## 7.1 核心开源工具创新点矩阵

| 工具/论文 | 来源 | 创新点 | 当前工具状态 | 可借鉴价值 | 优先级 |
|----------|------|--------|-------------|-----------|--------|
| **TMRG** (2017) | CERN | 注释驱动约束(`// tmrg`)、信号级/模块级TMR控制、自动fanout cell、FSM四策略、PLAG布局约束 | 无注释约束 | **高** — 可实现细粒度控制 | **P0** |
| **TaMaRa** (2025) | YosysHQ | 逻辑锥逆向追踪、错误信号OR-tree聚合、多锥递归处理、时钟/复位特殊处理 | 基本错误信号 | **高** — 改善验证质量 | **P1** |
| **FT-Pilot** (2026) | 中科院计算所 | GNN→LLM管线、双知识库RAG、失败知识积累、结构化Prompt组装 | 已有GNN+LLM但无失败积累 | **高** — 提升LLM生成质量 | **P0** |
| **Johnson & Wirthlin** (2010) | BYU | 四种投票器算法(归约/分区/同步/CDC)、投票器不能切除的约束条件 | 仅有归约型 | **中** — 增加投票器多样性 | **P1** |
| **Benites & Kastensmidt** (2018) | — | CGTMR/FGTMR分类、逻辑锥证明方法 | 已有粗细粒度 | **中高** — 改进验证方法 | **P1** |
| **SpyDrNet-TMR** | BYU | 多种投票器类型、Yosys/F4PGA开源流程支持 | 单一投票器 | **中** — 投票器类型扩展 | **P2** |
| **Xilinx TMRTool** | 商业 | "投票器也要复制"消除单点故障 | 未处理投票器冗余 | **中** — 完善TMR结构 | **P2** |
| **Li 2019/2020** | — | NSGA-II多目标优化、贝叶斯优化PPA评估流程 | 已有基本NSGA-II | **中** — 深化多目标优化 | **P2** |
| **TLegUp** (2017) | — | HLS级四种投票器位置、综合优化会移除冗余TMR的警示 | 有综合检查 | **高** — 需要`keep`保护 | **P0** |

## 7.2 按功能领域细分的创新点借鉴

### 7.2.1 加固控制粒度 (来自TMRG)

**创新点**: TMRG通过`// tmrg triplicate/do_not_triplicate`注释实现在RTL源码中的细粒度加固控制。

**当前工具差距**:
- 当前所有加固控制通过GUI配置，无法在RTL源码中嵌入控制指令
- 不支持信号级别的豁免（do_not_triplicate）

**借鉴方案**:
```
// harden_strategy: tmr, ecc, dice
// harden_skip: non_critical_signal, debug_bus
// harden_module: module_name -> tmr_state
```
- 优点: 无需GUI即可精确控制，适合批量处理
- 与现有策略矩阵协同: 注释约束覆盖策略矩阵（用户明确指定时），否则由策略矩阵自动分配
- 工作量: 2天

### 7.2.2 综合保护机制 (来自TLegUp/TMRG)

**创新点**: TLegUp研究发现综合工具(Yosys/Vivado)会自动优化移除"冗余"的TMR逻辑。TMRG通过`set_dont_touch`和综合约束文件(.sdc)解决。

**当前工具差距**:
- 当前仅输出加固RTL代码，不附带任何综合约束
- 生成的DICE代码在综合时可能被优化合并

**借鉴方案**:
- 自动添加 `(* keep = "true" *)` 属性到加固后的关键信号
- 生成配套的 `.sdc` / `.xdc` 综合约束文件
- 在GUI中添加"生成综合约束"选项
- 对DICE寄存器添加 `(* keep, preserve *)` 双属性保护
- 工作量: 1天

### 7.2.3 LLM生成质量提升 (来自FT-Pilot)

**创新点**: FT-Pilot的失败知识积累机制记录每次生成失败的信息（策略名、错误类型），在后续prompt中注入历史失败模式作为警告。

**当前工具差距**:
- 当前Auto-Repair仅验证语法和功能，不积累失败经验
- LLM生成时没有历史失败信息的上下文

**借鉴方案**:
- 新增 `FailureKnowledgeBase` 类，持久化记录失败案例
- 在LLM prompt中注入历史失败模式警告
- 实现错误分类映射（语法错误/接口错误/功能错误）
- 与现有RAG知识库协同
- 工作量: 1.5天

### 7.2.4 FSM保护策略扩展 (来自TMRG)

**创新点**: TMRG提供4种FSM保护策略：
- `fsm01`: 状态寄存器三模 + 状态跳转三模（最高保护）
- `fsm02`: 仅状态寄存器三模（中高保护）
- `fsm03`: 单热编码（中等保护）
- `fsm04`: 状态机错误检测 + 安全恢复（低开销）

**当前工具差距**:
- 当前仅支持 `tmr_state` 和 `one_hot` 两种FSM策略

**借鉴方案**:
- 扩展 `STRATEGY_MATRIX` 增加 `fsm_safe`（安全恢复）、`fsm_hamming`（汉明编码）策略
- 增加对应的AST变换模板
- 调整FSM信号类型的策略权重
- 工作量: 2天

### 7.2.5 投票器多样性 (来自Johnson & Wirthlin/SpyDrNet-TMR)

**创新点**: 四种投票器算法各有适用场景：
- 归约型(Reducing): 标准三选二
- 分区型(Partitioning): 模块级独立TMR
- 同步型(Synchronisation): 在SCC切割点插入
- CDC型: 跨时钟域信号的特殊处理
...
### 7.2.6 DICE变体支持 (来自调研报告文献)

**创新点**: 纯RTL级DICE加固效果有限，但有多个变体可增加数据集多样性：
- DNURL (双节点恢复): 比DICE面积小，SEU免疫
- TNUDICE (三节点DICE): 支持三节点翻转恢复
- HLTNURL (三节点自恢复): 最高级保护
...
### 7.2.7 错误信号增强 (来自TaMaRa)

**创新点**: TaMaRa将所有投票器的`err`信号通过OR树汇聚为顶层错误标志。
...
| 阶段 | 任务 | 来源参考 | 工作量 | 优先级 | 状态 |
|------|------|---------|--------|--------|------|
| **Phase 1** | 注释驱动约束系统 | TMRG | 2天 | **P0** | ✅ |
|  | 综合保护机制(keep/sdc) | TLegUp/TMRG | 1天 | **P0** | ✅ |
|  | 故障知识积累 | FT-Pilot | 1.5天 | **P0** | ✅ |
| **Phase 2** | FSM保护策略扩展(fsm_hamming/fsm_safe) | TMRG | 2天 | **P1** | ✅ |
|  | 错误信号OR-tree | TaMaRa | 1天 | **P1** | ✅ |
|  | DICE变体支持(dnurl/tnudice) | 报告文献 | 1.5天 | **P1** | ✅ |
| **Phase 3** | 投票器多样性(partitioning/sync/cdc) | Johnson/SpyDrNet | 3天 | **P2** | ✅ |
|  | CDC信号检测与加固 | Johnson | 1.5天 | **P2** | ✅ |
|  | PPA多目标评估深化 | Li 2019/2020 | 2天 | **P2** | ✅ |
|  | GUI完整集成(Phase0-3全部选项) | 自研 | 2天 | **P2** | ✅ |

## 7.5 创新点实现状态总览（vs 调研报告）

下表完整展示当前工具 v5.2 对调研报告中各开源工具和论文创新点的实现状态：

| 工具/论文 | 创新点 | 状态 | 实现位置 | 评估 |
|---------|-------|:----:|---------|:----:|
...
| 功能 | 工具来源 | 不采纳原因 |
|------|---------|-----------|
| RASP-TMR全模块强制TMR | Kassel大学 | 太粗粒度，已有灵活的策略矩阵 |
| SpyDrNet网表级TMR | BYU | 非RTL级，与工具定位不符 |
| TaMaRa RTLIL层操作 | YosysHQ | 信息有损，不适合RTL数据集生成 |
| TLegUp HLS级TMR | — | HLS流程不同，仅警示参考 |
| Xilinx TMRTool | 商业 | 闭源，不可集成 |
