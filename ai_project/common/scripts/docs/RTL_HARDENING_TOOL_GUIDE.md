...
|--------|--------|------|-----------|---------|
| P1 | **Windows gbk编码根治** | 全局UTF-8环境变量+二进制读取+UTF-8临时文件兼容方案 | 2天 | ✅ 已根治，pyverilog兼容层完成 |
| P1 | **oss-cad-suite自动安装修复** | 新增搜索路径(PROGRAMFILES/LOCALAPPDATA)、安装后版本验证、目录结构诊断 | 1天 | ✅ 已修复 |
| P2 | **大规模设计性能优化** | 并行信号处理(ThreadPoolExecutor分批)、并行模式开关、性能统计 | 3天 | ✅ 已实现 |
| P2 | **CI/CD Windows测试矩阵** | GitHub Actions配置ubuntu+windows × 3.9/3.10/3.11矩阵 | 1天 | ✅ 已配置 |
| P2 | **更多数据集格式支持** | RTLCoder/ChipNeMo/VeriGen格式 | 2天 | ✅ 已支持RTLCoder格式 |
| P3 | **加固效果可视化增强** | 雷达图(plot_radar_chart)、趋势图(plot_trend_chart)、多设计对比雷达图 | 2天 | ✅ 已实现 |
| P3 | **GUI界面主题支持** | THEMES字典(light/dark)+_apply_theme递归遍历+主题切换Combobox | 1天 | ✅ 已实现 |
| P3 | **一键升级功能** | check_for_updates(GitHub API)+add_update_button+底部版本状态栏 | 2天 | ✅ 已实现 |
| P3 | **Linux平台完整移植** | Docker已就绪，CI/CD已配置，Web GUI可选 | 2.5天 | ✅ Docker就绪 |
...
| 阶段 | 任务 | 来源参考 | 工作量 | 优先级 |
|------|------|---------|--------|--------|
| **Phase 1** (本周) | 注释驱动约束系统 | TMRG | 2天 | **P0** |
|  | 综合保护机制(keep/sdc) | TLegUp/TMRG | 1天 | **P0** |
|  | 故障知识积累 | FT-Pilot | 1.5天 | **P0** |
| **Phase 2** (下周) | FSM保护策略扩展 | TMRG | 2天 | **P1** |
|  | 错误信号OR-tree | TaMaRa | 1天 | **P1** |
|  | DICE变体支持 | 报告文献 | 1.5天 | **P1** |
| **Phase 3** (后续) | 投票器多样性 | Johnson/SpyDrNet | 3天 | **P2** |
|  | 多目标优化深化 | Li 2019/2020 | 2天 | **P2** |
|  | CDC信号加固 | Johnson | 1.5天 | **P2** |
|  | 逻辑锥验证 | Benites | 2天 | **P3** |
