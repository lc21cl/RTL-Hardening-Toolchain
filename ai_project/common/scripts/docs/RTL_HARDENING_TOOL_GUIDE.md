...
| 优先级 | 优化项 | 说明 | 状态 |
|--------|--------|------|------|
| P3 | **逻辑锥验证集成** (Benites & Kastensmidt) | 基于逻辑锥的等价性验证方法 | 🔴 未开始 |
| P3 | **一键升级功能** | GUI自动检测GitHub新版本 | 🔴 未开始 |

> 其余优化项（注释约束、综合保护、故障知识积累、FSM扩展、DICE变体、错误OR-tree、
> 投票器多样性、CDC检测、PPA评估）均已在 v5.0 中实现并集成到GUI。
