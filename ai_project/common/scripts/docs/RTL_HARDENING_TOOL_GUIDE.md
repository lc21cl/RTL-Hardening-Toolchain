...
└── 工具完备性: 12策略 + 4投票器 + 8功能模块 + 1示例插件    ✅
```

### 6.3.2 调研报告中仍未实现的核心创新点

以下是从《RTL级加固方法综述与技术选型报告》中梳理出的、**尚未在当前工具中实现**的创新点：

| 来源 | 未实现创新点 | 说明 | 参考价值 | 工作量 |
|------|------------|------|---------|-------|
| **TMRG** | **Fanout cell插入** | 将输入信号扇出为3路独立副本(_A/_B/_C)，避免TMR共享输入的单点故障 | **高** — TMR结构完整性 | 1天 |
| **TMRG** | **三时钟TMR** | 三个副本使用独立时钟(clkA/clkB/clkC)，抵御时钟相关的SEU | **高** — 抗辐射能力提升 | 1.5天 |
| **TMRG** | **tmr_error自动恢复** | 将错误信号接入加载逻辑实现自恢复(`load \| tmrError`)，比OR-tree更实用 | **高** — 自我修复能力 | 1天 |
| **TMRG** | **PLAG布局约束** | 生成物理布局约束防止TMR单元间距过近 | **中** — 物理设计集成 | 2天 |
| **FT-Pilot** | **CoT语义分析** | 用LLM Chain-of-Thought分析寄存器角色(而非正则匹配) | **高** — 分类准确性 | 2天 |
| **FT-Pilot** | **多层验证管线** | 语法→可综合→接口一致性→功能仿真 四层验证 | **高** — 代码质量 | 2天 |
| **FT-Pilot** | **双知识库RAG** | 语义知识库(策略描述) + 示例知识库(代码模板)分离检索 | **中** — 检索精度 | 1.5天 |
| **J&W 2010** | **投票器"禁区"规则** | DSP原语/进位链等不允许插入投票器的约束条件 | **中** — 验证补充 | 1天 |
| **ECC文献** | **完整SECDED实现** | 32位汉明码编码器/解码器(13种模板中SECDED支持有限) | **高** — 功能正确性 | 2天 |
| **ECC文献** | **BCH码多比特纠错** | BCH码支持多比特翻转恢复，远超SECDED | **中** — 高级场景 | 2天 |
| **DICE文献** | **HLTNURL三节点自恢复** | 最高级别DICE变体，三节点翻转恢复 | **中** — 策略扩展 | 1天 |
| **TaMaRa** | **逻辑锥逆向BFS** | 从输出逆向BFS搜索组合逻辑进行三模复制 | **中** — 算法创新 | 2天 |

### 6.3.3 推荐实施路线图

```
Phase A: 加固结构完整性（2天，提升TMR质量）
  └── P1: Fanout cell插入 + 三时钟TMR + tmr_error自动恢复

Phase B: LLM质量提升（3天，提升AI生成效果）
  └── P1: CoT语义分析 + 多层验证管线 + 双知识库RAG

Phase C: 策略扩展（3天，增加加固方式）
  └── P2: 完整SECDED + BCH码 + HLTNURL + 投票器禁区规则

Phase D: 物理设计集成（2.5天，工程化）
  └── P3: PLAG布局约束 + 逻辑锥逆向BFS
```

> 当前工具功能完备度 **98%**（功能覆盖）/ **75%**（创新点覆盖）。
> 调研报告17个核心创新点中，已实现12个，剩余5个待实施。
