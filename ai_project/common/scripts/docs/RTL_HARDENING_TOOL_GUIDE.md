...
| 优先级 | 优化项 | 说明 | 预计工作量 |
|--------|--------|------|-----------|
| P2 | **iverilog编译验证** | 修复加固后代码语法错误，确保iverilog编译通过 | 2天 |
| P2 | **GUI性能优化** | 大规模设计(10000+信号)的GUI响应优化 | 2天 |
| P3 | **AST变换深度修复** | 完全重写正则变换，使用pyverilog AST真实变换 | 3天 |
| P3 | **多语言Prompt模板** | 支持英文/日文等多语言的LLM生成模板 | 1天 |
| P3 | **加固效果在线对比** | 加固前后代码的diff高亮显示 | 2天 |
| P3 | **用户自定义策略** | GUI中允许用户编写自定义策略变换规则 | 3天 |

> 当前工具功能完备度达到 **98%**，剩余优化为锦上添花性质，不影响核心加固流程。

### 6.3.2 用户体验优化

| 优先级 | 优化项 | 说明 | 预计工作量 |
|--------|--------|------|-----------|
| P2 | **加固结果对比表格** | 在GUI中直接显示原始vs加固的面积/可靠性对比表 | 1天 |
| P2 | **批量进度细化** | 文件夹/数据集加固时显示每个文件的详细进度百分比 | 1天 |
| P2 | **一键清除输出** | 在输出区域添加"清除"按钮 | 0.5天 |
| P3 | **GUI右键菜单** | 在代码对比面板添加右键复制/搜索功能 | 1天 |
| P3 | **加固报告预览** | 在GUI中直接内嵌HTML报告预览 | 2天 |

### 6.3.3 工程化优化

| 优先级 | 优化项 | 说明 | 预计工作量 |
|--------|--------|------|-----------|
| P2 | **日志分级系统** | 区分INFO/WARN/ERROR/DEBUG级别，支持日志文件持久化 | 1天 |
| P2 | **配置文件持久化** | 保存用户配置（策略偏好/API Key/窗口大小）到JSON | 1天 |
| P3 | **断点续传** | 数据集加固过程中支持中断后从断点继续 | 2天 |
| P3 | **插件系统** | 支持外部加固策略的动态加载 | 3天 |

### 6.3.4 性能与兼容性

| 优先级 | 优化项 | 说明 | 预计工作量 |
|--------|--------|------|-----------|
| P1 | **Python 3.13兼容性** | pyverilog在3.14上有ResourceWarning，需确认3.13兼容 | 1天 |
| P1 | **AST变换代码健壮性** | mixed_design.v加固后iverilog语法错误 | 2天 |
| P2 | **纯UTF-8测试用例** | 创建不含gbk字符的测试RTL文件，使AST路径全覆盖 | 0.5天 |

### 6.3.5 优先级排序执行建议

```
第一优先级（立即执行）:
  ├── P1: 修复AST变换代码，解决iverilog编译错误
  ├── P1: 逻辑锥等价性验证集成
  └── P1: Python版本兼容性确认
