    ...
    def __init__(self, model, quantized_params):
        ...
        for name, param in quantized_params.items():
            if param['type'] == 'weight':
                ...
                self.dequantized_weights[name] = torch.tensor(param['data'], dtype=torch.float32)
...
def test_quantization():
    print('=' * 70)
    print('  INT8 Quantization Accuracy Test - SAGE2-Lite-64')
    print('=' * 70)
    ...
    print('\n  --- Quantizing Model (INT8 Weight Only) ---')
    quantizer = INT8Quantizer()
    quantized_params = quantizer.quantize_model(model)

    quant_path = os.path.join(OUTPUT_DIR, 'quantized_weights_sage2_lite_64.json')
    ...
    print(f'    TP/FP/FN/TN: {fp32_metrics["tp"]}/{fp32_metrics["fp"]}/{fp32_metrics["fn"]}/{fp32_metrics["tn"]}')
    ...
    print(f'    TP/FP/FN/TN: {int8_metrics_same_th["tp"]}/{int8_metrics_same_th["fp"]}/{int8_metrics_same_th["fn"]}/{int8_metrics_same_th["tn"]}')
    ...
    print(f'    TP/FP/FN/TN: {int8_metrics_own_th["tp"]}/{int8_metrics_own_th["fp"]}/{int8_metrics_own_th["fn"]}/{int8_metrics_own_th["tn"]}')
    ...
    print(f'\n    Overall:            {"✅ ALL CHECKS PASSED - Quantization is acceptable!" if all_pass else "❌ SOME CHECKS FAILED - Needs optimization"}')
    ...
    result_path = os.path.join(OUTPUT_DIR, 'int8_quantization_test_results.json')
